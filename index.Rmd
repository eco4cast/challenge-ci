...
combined <- reactive({
  ds %>% filter(theme == input$forecast) %>% collect()
})

potential_dates <- reactive({
  df2 <- unique(combined()$start_time)
  df2 <- lubridate::as_date(sort(df2, decreasing = TRUE))
  df2
})
...
potential_facets <- reactive({
  df <- team_date_forecast() %>% 
    dplyr::select("site_id", "x", "y", "z") %>% 
    mutate(x = as.character(x),
           y = as.character(y),
           z = as.character(z)) %>% 
    pivot_longer(cols = c("site_id", "x", "y", "z"), names_to = "variable", values_to = "value") %>% 
    filter(!is.na(value))
  unique(df$variable)
})
...
combined2 <- reactive({
    ds %>% filter(theme == input$forecast2) %>% collect()
...
potential_dates2 <- reactive({
  df2 <- combined2() %>% filter(model_id == input$team2)
  df2 <- unique(df2$start_time)
  lubridate::as_date(sort(df2, decreasing = TRUE))
})
