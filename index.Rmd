...
combined2 <- reactive({
  ...
  s3_2 <- paste0(challenge_config$DATA_DIR, "/neon4cast-scores/parquet/",input$forecast)
...
sites2 <- reactive({
  df <- combined2() %>% filter(lubridate::as_date(start_time) == lubridate::as_date(input$date2),
                              model_id == input$team2)
  unique(df$site_id)
})
